...
class StudentAPI {
  private url: string = "https://apiestudiantes.maosystems.dev/estudiantes";
  private headers: HeadersInit = {
    "Content-Type": "application/json",
    Authorization:
      "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZGVudGlmaWNhY2lvbiI6MSwiY29ycmVvIjoidXNlcjEiLCJpYXQiOjE2ODI4MTEzODQsImV4cCI6MTY4MzQxNjE4NH0.a9TvrAbBSsfPDbZXibv2SgdUSpNY5FppWDklUHSpxV8",
  };
  private modal: HTMLDialogElement;

  constructor() {
    this.modal = document.querySelector(SELECTOR_MODAL) as HTMLDialogElement;
  }
  ...
  renderStudents(students: { message: string; data: Student[] }) {
    const database = students.data;
    const tbody = document.querySelector("tbody");

    if (tbody) {
      tbody.innerHTML = "";
      database.forEach((student: Student) => {
        tbody.innerHTML += `
              <tr>
      ...
      fetch(this.url, {
        ...
        .then((response) => {
          ...
          if (response.status === 400)
            throw alert(
              `Error creando el estudiante,  verifique los datos - ${response.status}`
            );
        })
        .then(() => {
  ...
  editStudent() {
    document.addEventListener("click", (event) => {
      if ((event.target as HTMLInputElement).classList.contains("edit")) {
  ...
  extractStudentDataFromRow(row: HTMLElement) {
  ...
  fillFormWithData(data: EditStudent) {
    const form = document.querySelector(SELECTOR_MODAL_FORM) as HTMLFormElement;
    if (form) {
    ...
    try {
      fetch(`${this.url}/${id}`, {
        ...
        .then((response) => {
          if (response.status === 400)
            throw alert(
              `Error actualizando el estudiante, verifique los datos - ${response.status}`
            );
        })
        .then(() => {
  ...
  init() {
    const addStudent = document.querySelector(SELECTOR_ADD_STUDENT_BTN) as HTMLDialogElement;
    const form = document.querySelector(SELECTOR_MODAL_FORM) as HTMLFormElement;
    const tipoIdentificacionInput = document.querySelector(SELECTOR_TIPO_IDENTIFICACION) as HTMLInputElement;
    ...
    addStudent.addEventListener("click", () => {
      (form.querySelector("h4") as HTMLHeadingElement).textContent =
        "Agregar Estudiante";
    ...
    form.addEventListener("submit", (event) => {
      ...
      const data: NewStudent | EditStudent = {
      ...
      if ((data as EditStudent).identificador) {
        const editData = data as EditStudent;
        this.updateStudent(editData, Number(editData.identificador));
      } else {
        const newStudentData = data as NewStudent;
